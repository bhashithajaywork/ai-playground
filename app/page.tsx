...
import { 
  ...
  Terminal, Zap, LayoutDashboard, History, Activity
} from 'lucide-react';
...
export default function Home() {
  ...
  const tools = [
    ...
    { id: 'code', label: 'Code Explainer', icon: Code2, placeholder: 'Paste your code snippet here to explain...', quick: 'const sum = (a, b) => a + b;' },
    { id: 'meme', label: 'Meme Ideas', icon: Laugh, placeholder: 'Enter a topic or situation for viral meme ideas...', quick: 'Debugging code in production at 3 AM' },
    { id: 'summary', label: 'Summarizer', icon: FileText, placeholder: 'Paste long text or essay to summarize...', quick: 'Artificial Intelligence and Machine Learning overview...' },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans selection:bg-purple-500 selection:text-white">
      ...
      <main className="max-w-7xl mx-auto w-full p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 items-start">
        ...
        {/* LEFT PANEL: TOOL SELECTOR & STATS */}
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-4 space-y-3">
            <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-2">Select AI Module</h2>
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-4 space-y-2">
            ...
            <p className="text-xs text-slate-400">Click to load sample prompt:</p>
            <button 
          ...
          {/* Engine Specs */}
          <div className="bg-slate-900/30 border border-slate-800/50 rounded-2xl p-4 text-xs text-slate-500 space-y-2 font-mono">
            <div className="flex justify-between"><span>Model Architecture:</span> <span className="text-slate-300">Llama3 / Mixtral</span></div>
            <div className="flex justify-between"><span>Response Latency:</span> <span className="text-emerald-400">~200ms</span></div>
            <div className="flex justify-between"><span>System Status:</span> <span className="text-slate-300">Operational</span></div>
          </div>
        </div>

        {/* RIGHT PANEL: MAIN INTERACTION & WORKSPACE */}
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 shadow-2xl space-y-5">
            ...
            <textarea
              ...
              className="w-full h-40 p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500/80 resize-none transition-all placeholder:text-slate-600 text-sm leading-relaxed"
            />
            ...
            <button
              ...
              className="w-full py-4 rounded-xl bg-gradient-to-r from-purple-600 via-pink-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold shadow-lg shadow-purple-600/25 disabled:opacity-50 transition-all flex items-center justify-center gap-2 active:scale-[0.99] cursor-pointer"
            >
              {loading ? (
                <>
            ...
            <AnimatePresence>
              {result && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -12 }}
                  className="p-5 rounded-xl bg-slate-950 border border-purple-500/30 space-y-3 relative group shadow-inner"
                >
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <span className="text-xs font-semibold text-purple-400 uppercase tracking-wider flex items-center gap-1.5">
                      <Terminal className="w-3.5 h-3.5" /> Generated Response
                    </span>
                    <button
                      onClick={copyToClipboard}
                      className="flex items-center gap-1.5 text-xs text-slate-300 hover:text-white bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800 transition-all"
                    >
                      {copied ? (
                        <>
                  ...
                  <div className="text-slate-200 whitespace-pre-wrap leading-relaxed text-sm font-sans">
                    {result}
                  </div>
                </motion.div>
              )}
      ...
      {/* 3. FOOTER */}
      <footer className="w-full border-t border-slate-800/80 bg-slate-950 py-6 px-6 text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          © {new Date().getFullYear()} <span className="text-slate-300 font-semibold">AI Playground</span>. All rights reserved. Powered by Jay Solutions.
        </div>
        <div className="flex items-center gap-6">
          <span className="hover:text-slate-300 cursor-pointer">Privacy Policy</span>
          <span className="hover:text-slate-300 cursor-pointer">Terms of Service</span>
          <span className="hover:text-slate-300 cursor-pointer">API Status</span>
        </div>
      </footer>
